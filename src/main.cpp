...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < groupDim; ++i) 
    {
        ...
        // Experiment for varying Epsilon and Lambda
        double deltaC = 1;
        int setDelta = 1;
        ...
        int j = 0;
        for(double epsilon = 0.01; epsilon < 0.802; epsilon += 0.05)
        {
            MHRsEL.push_back({});
            CPUTimesEL.push_back({});
            for(double lambda = 0.01; lambda < 0.802; lambda += 0.05)
            {   
                result.clear();
                time = -1.0;
                cout << "Running Bi-Greedy-Plus..." << endl;
                runBiGreedyPlusAlgWithDelta(groupedDataP, fairnessConstraint, dataP, i, lambda, epsilon, utiFunClass, result, k, maxM, time, deltaC);
                writeToFileELD(dataP, dim, groupToInt, fairnessConstraint, argv[1], result, k, i, "Bi-Greedy-Plus: ", time, MHR, epsilon, lambda, deltaC, setDelta);
                MHRsEL[j].push_back(MHR);
                CPUTimesEL[j].push_back(time);
                if(lambda - 0.01 < 1e-9)
                    lambda = 0;
            }
            if(epsilon - 0.01 < 1e-9)
                epsilon = 0;
            j++;
        ...
        outFileName += argv[1];
        outFileName = outFileName.substr(0, outFileName.length() - 4);
        outFileName += "_" + to_string(k) + "_DC" + to_string(deltaC).substr(0,4) + ".data";
        ...
        j = 0;
        for(double deltaC = 1.25; deltaC < 50; deltaC *= 2)
        {
            result.clear();
            time = -1.0;
            cout << "Running Bi-Greedy..." << endl;
            runBiGreedyAlgWithDelta(groupedDataP, fairnessConstraint, dataP, i, epsilon, utiFunClass, result, k, maxM, time, deltaC);
            writeToFileELD(dataP, dim, groupToInt, fairnessConstraint, argv[1], result, k, i, "Bi-Greedy: ", time, MHR, epsilon, lambda, deltaC, setDelta);
            MHRs[j] = MHR;
            CPUTimes[j] = time;
        ...
        outFileName += argv[1];
        outFileName = outFileName.substr(0, outFileName.length() - 4);
        outFileName += "_" + to_string(k) + "_E" + to_string(epsilon).substr(0,4) + "_L" + to_string(lambda).substr(0,4) + ".data";
