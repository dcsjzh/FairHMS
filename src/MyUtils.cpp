...
vector<string> split(string src, char ch)
{
    ...
    if(tmpString != "") splitString.push_back(tmpString);
    return splitString;
}
                 ...
                 string datasetPath, vector<int> &result, int k, int groupID, string algName, double time, double &MHR)
{
    ...
    else
    {
        ...
        result4c = RMSUtils::pointSetTransf(resultP);
        MHR = 1 - evaluateLP(fatP4c, result4c, 0);
    }
                    ...
                    string datasetPath, vector<int> &result, int k, int groupID, string algName, double time, double &MHR, double epsilon, double lambda, double deltaC, int setDelta)
{
    ...
    outFileName = outFileName.substr(0, outFileName.length() - 4);
    if(setDelta == 1)
        outFileName += "_" + to_string(k) + "_DC" + to_string(deltaC).substr(0,4) + ".txt";
    else
        outFileName += "_" + to_string(k) + "_E" + to_string(epsilon).substr(0,4) + "_L" + to_string(lambda).substr(0,4) + ".txt";
    ofstream fout(outFileName, ios::app);
    if(setDelta == 1)
        fout << algName << "groupID=" << groupID << "\tEpsilon=" << epsilon << "\tLambda"  << lambda << endl
            << "Fairness constraint: ";
    else
        fout << algName << "groupID=" << groupID << '\t' << "DeltaC=" << deltaC << endl
            << "Fairness constraint: ";
