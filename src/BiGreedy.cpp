                          ...
                          int k, int maxM, double &time, double deltaC)
{
      ...
      biSearchResult(groupedDataP, fairnessConstraint, dataP, groupID, epsilon, utiFunClass,
                         result, maxUtilityofD, maxUtilityofS, tauHigh, tauLow, tau, fairofResult, k, m, gamma);
      int times = 2;
      if(deltaC == 0.25) times *= 2;
      while (true)
      {
            m *= times;
            m  = min(m, maxM);
            for (int i = m / times; i < m; ++i)
            {
                  double maxUtility = 0.0;
                  int maxIdx = -1;
                  for (Point p : dataP)
            ...
            multiroundGreedy(groupedDataP, fairnessConstraint, dataP, groupID, epsilon, utiFunClass, tmpResult,
                             maxUtilityofD, tmpMaxS, max(tau - lambda, 0.001), tmpFairofResult, k, m, gamma, sumDelta);
            if (tmpResult.size() != 0)
            {
                  biSearchResult(groupedDataP, fairnessConstraint, dataP, groupID, epsilon, utiFunClass,
                                     result, maxUtilityofD, maxUtilityofS, tau, max(tau - lambda, 0.001), tau, fairofResult, k, m, gamma);
                  break;
            }
            else
            {
                  biSearchResult(groupedDataP, fairnessConstraint, dataP, groupID, epsilon, utiFunClass,
                                     result, maxUtilityofD, maxUtilityofS, max(tau - lambda, 0.001), 0, tau, fairofResult, k, m, gamma);
            }
      }
      clock_t end = clock();
      time = (double)(end - start) / (CLOCKS_PER_SEC / 1000);
