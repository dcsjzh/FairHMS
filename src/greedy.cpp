...
{
  cout << "Running Matroid-Greedy..." << endl;
  clock_t tstart = clock();
  ...
  ++fairofResult[fatP[maxIndex].get_category(groupID)];
  R.push_back(fatP[maxIndex]);
  int oldIdx = maxIndex;
  vector<int> candidate = constructCandidate(groupedDataP, fairnessConstraint, result, fairofResult, r);
  ...
  while (idxs.size() < r)
  {
    ...
    for (size_t i = 0; i < fatP.size(); i++)
    {
      if (find(idxs.begin(), idxs.end(), i) != idxs.end() || find(candidate.begin(), candidate.end(), i) == candidate.end())
      {
        continue;
      }
      GRBModel model = GRBModel(env);
    ...
    if (find(idxs.begin(), idxs.end(), id) != idxs.end()) break;
    idxs.push_back(id);
    result.push_back(id);
    ++fairofResult[fatP[id].get_category(groupID)];
    R.push_back(fatP[id]);
    candidate.clear();
    candidate = constructCandidate(groupedDataP, fairnessConstraint, result, fairofResult, r);
  }

  while (idxs.size() < r)
  {
    int id = candidate[rand()%candidate.size()];
    if (find(idxs.begin(), idxs.end(), id) != idxs.end()) continue;
    idxs.push_back(id);
    result.push_back(id);
    ++fairofResult[fatP[id].get_category(groupID)];
    R.push_back(fatP[id]);
